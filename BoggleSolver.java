...
public class BoggleSolver
{
	...
	private boolean isValid(int row, int col, int direction){
		...
		//System.out.println("The current row and col we are checking {" +row + ","+col+"}" );
		//TODO: Write the code to check whether already used / at an edge of the board
		//System.out.println(row>=0 && row<= theBoard.length-1 && col>=0 && col<= theBoard.length-1 && !Character.isUpperCase(theBoard[row][col]));
		if(coords.row>=0 && coords.row<= theBoard.length-1 && coords.col>=0 && coords.col<= theBoard.length-1 && !Character.isUpperCase(theBoard[coords.row][coords.col])) return true;
		else return false;
	}
